    ...
    <div class="xl:col-span-3" role="region" aria-label="Ticket queue">
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <div class="border-b border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-800">
                ...
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Sorted by priority and SLA deadline</p>
            </div>
                    ...
                    >
                        <div class="flex items-start justify-between gap-2">
                            <div class="flex min-w-0 flex-col">
                                ...
                                <p class="mt-1 line-clamp-2 text-xs leading-snug text-gray-700 dark:text-gray-300">{{ $ticket['subject'] }}</p>
                            </div>
                        </div>
        ...
        <div class="flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            @if ($selectedTicket)
                ...
                <ol class="flex-1 max-h-72 overflow-y-auto space-y-1 px-4 py-3" aria-label="Conversation timeline">
                    @forelse ($timeline as $entry)
                        ...
                        <li class="rounded-lg border p-3 text-sm {{ $bubbleBg }}" tabindex="0">
                            ...
                            <p class="whitespace-pre-wrap leading-relaxed text-gray-800 dark:text-gray-200">{{ $entry['message'] }}</p>
                        </li>
                    @empty
                        <li class="flex flex-col items-center justify-center py-8 text-center text-sm text-gray-400">
                            <x-filament::icon icon="heroicon-o-chat-bubble-left-right" class="mb-2 h-7 w-7 text-gray-300" />
                            No messages yet
                        </li>
                    @endforelse
                </ol>
                ...
                <div class="space-y-3 border-t border-gray-200 p-4 dark:border-gray-700">
                    ...
                    <div class="flex items-center justify-end gap-2">
                        @if (! $this->canReply())
                            <p class="text-xs text-amber-600 dark:text-amber-400">View-only access: reply permission is required to respond.</p>
                        @endif
                        <x-filament::button
                            color="success"
            ...
            @else
                <div class="flex flex-col items-center justify-center px-6 py-24 text-center">
                    <x-filament::icon icon="heroicon-o-inbox-stack" class="mb-3 h-12 w-12 text-gray-300 dark:text-gray-600" />
    ...
    <div class="xl:col-span-3 space-y-4" role="complementary" aria-label="Ticket context">
        @if ($selectedTicket)
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                ...
                <dl class="divide-y divide-gray-100 text-xs dark:divide-gray-800">
                    <div class="flex items-center justify-between px-4 py-2.5">
                        <dt class="text-gray-500 dark:text-gray-400">Requester</dt>
                        <dd class="max-w-[60%] truncate text-right font-medium text-gray-800 dark:text-gray-200">{{ $selectedTicket['requester_email'] ?? '-' }}</dd>
                    </div>
                    <div class="flex items-center justify-between px-4 py-2.5">
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($relatedTickets as $related)
                    <div class="px-4 py-3">
                        ...
                        <p class="mt-1 line-clamp-2 text-xs text-gray-600 dark:text-gray-300">{{ $related['subject'] }}</p>
                        ...
                        @if (!empty($related['resolution_summary']))
                            <p class="mt-1.5 line-clamp-2 text-[11px] italic text-gray-500 dark:text-gray-400">
