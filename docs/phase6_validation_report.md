...
| 6.2 CRM intake hardening (captcha + throttling + honeypot) | Complete | Captcha validation integrated for `/api/preregister` and `/api/support/ticket`; legacy `/api/mobcontact` returns deprecation response with intake protections retained. |
