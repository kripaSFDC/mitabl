...
### Phase 1 (1–2 sprints): foundation and safety
1. Introduce shared API client + typed response envelope.
2. Remove secret-bearing logs and replace with redacted logger.
3. Add request timeout + retry/backoff policy for idempotent reads.
4. Replace force-unwrapped route arguments in critical screens.

### Phase 2 (1 sprint): home feed performance and relevance
1. Integrate real location flow with permission fallback state.
2. Debounce filter events and cancel previous requests.
3. Convert home screen to sliver/lazy sections.
4. Add empty states for each feed section (not just spinners).

### Phase 3 (ongoing): UX polish and observability
1. Functionalize search/location control or simplify UI affordance.
2. Introduce screen-level metrics (TTI, API latency buckets, failure rates).
3. Add widget/integration tests around feed loading/error/empty states.

## Quick win checklist
- [ ] Remove token/response prints.
- [ ] Move `SystemChrome` side effect out of hot rebuild path.
- [ ] Add null-safe guard around settings route args.
- [ ] Implement minimum request timeout to prevent hanging operations.
- [ ] Add actionable error snackbars with retry entry points.
